...
LOG_EMOCOES = DATASET_EMOCOES / "registro_gravacoes.csv"
...
def entrada_audio(key, dur, texto_botao="Gravar"):
    """Microfone do PC, gravador do navegador ou arquivo WAV. Retorna np.array 16 kHz ou None."""
    ...
    if modo == modos[0]:
        ...
        if st.button(texto_botao, key=f"{key}_btn", type="primary", icon=":material/mic:"):
            try:
                ...
                st.error(f"Não consegui usar o microfone: {exc}. Use o 'Gravador do navegador'.")
    elif modo == modos[1]:
        up = st.audio_input("Clique no microfone, fale e clique de novo para parar", key=f"{key}_ai")
        if up is not None:
            st.session_state[f"{key}_audio"] = carregar_audio(up)
    else:
        up = st.file_uploader("Selecione um arquivo .wav", type=["wav"], key=f"{key}_up")
        if up is not None:
            st.session_state[f"{key}_audio"] = carregar_audio(up)
    return st.session_state.get(f"{key}_audio")


def qualidade(y, cfg):
    n = analisar_nivel(y, cfg)
    if eh_silencio(y, cfg):
        st.warning(f"Áudio muito baixo ou vazio (fala detectada em {n['fracao_fala']:.0%} do tempo). Grave novamente.",
...
def bloco_salvar(key, y, cfg, destino_fn):
    st.audio(y, sample_rate=SAMPLE_RATE)
    bom = qualidade(y, cfg)
    c1, c2, _ = st.columns([1, 1, 3])
    if c1.button("Salvar no dataset", key=f"save_{key}", type="primary", disabled=not bom, icon=":material/check:"):
        nome = destino_fn(y)
        st.session_state.pop(f"{key}_audio", None)
        st.toast(f"Salvo: {nome}", icon=":material/check_circle:")
        st.rerun()
...
def pagina_datasets():
    ...
    with aba_p:
        ...
        if classes:
            ...
            y = entrada_audio("rec_p", dur, "Gravar frase")
            if y is not None:
                def salvar(y):
                    path = proximo_arquivo(DATASET_PESSOAS / pessoa, pessoa)
                    salvar_wav(path, y)
                    return path.name
                bloco_salvar("rec_p", y, cfg, salvar)
    ...
    with aba_e:
        ...
        with c2:
            log = pd.read_csv(LOG_EMOCOES) if LOG_EMOCOES.exists() else pd.DataFrame()
            if len(log):
        ...
        frase = cfg["frases"][counts.get("neutro", 0) % len(cfg["frases"])]
        card(f'<div class="row"><span class="card-title">Emoção: {emocao}</span>'
             ...
             f'<div class="frase">“{esc(frase)}”</div>')
        y = entrada_audio("rec_e", dur, "Gravar frase")
        if y is not None:
            def salvar_e(y):
                path = proximo_arquivo(DATASET_EMOCOES / emocao, emocao)
                salvar_wav(path, y)
                novo = not LOG_EMOCOES.exists()
                with LOG_EMOCOES.open("a", newline="", encoding="utf-8") as f:
                    w = csv.writer(f)
                    if novo:
                        w.writerow(["arquivo", "emocao", "pessoa", "data"])
                    w.writerow([path.name, emocao, quem, datetime.now().strftime("%Y-%m-%d %H:%M:%S")])
                return path.name
            bloco_salvar("rec_e", y, cfg, salvar_e)
...
def pagina_testar():
    ...
    y = entrada_audio("teste", cfg["duracao_gravacao"], "Gravar e classificar")
    if y is None:
        return
    st.audio(y, sample_rate=SAMPLE_RATE)
...
def pagina_config():
    ...
    with st.form("cfg"):
        ...
        cfg["limiar_silencio_rms"] = c1.slider("Sensibilidade de silêncio (energia RMS)", 0.002, 0.05,
    ...
    st.markdown("### Teste de ruído da sala")
    st.caption("Grava 3 segundos em silêncio para conferir se o limiar de silêncio está adequado.")
    if st.button("Medir ruído", icon=":material/graphic_eq:"):
        try:
            y = gravar_trecho(3.0)
            n = analisar_nivel(y, cfg)
            st.write(f"RMS do ruído: **{n['rms']:.4f}** · limiar atual: {cfg['limiar_silencio_rms']:.3f}")
            if n["rms"] > cfg["limiar_silencio_rms"] * 0.7:
                st.warning("Ruído perto do limiar. Aumente a sensibilidade de silêncio ou grave em um lugar mais quieto.")
            else:
                st.success("Ambiente adequado.")
        except Exception as exc:
